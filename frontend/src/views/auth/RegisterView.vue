<template>
  <div class="min-h-screen bg-gradient-to-br from-brand-700 to-brand-900 flex items-center justify-center p-4">
    <div class="w-full max-w-md">
      ...
      <div class="card p-8">
        <h1 class="text-xl font-semibold text-gray-900 mb-6">Konto erstellen</h1>
        <form @submit.prevent="handleRegister" class="space-y-4">
          <div>
            <label class="label">Vollständiger Name</label>
            <input v-model="form.full_name" type="text" class="input" placeholder="Max Mustermann" required />
          </div>
          <div>
          ...
          <div>
            <label class="label">Unternehmen <span class="text-gray-400">(optional)</span></label>
            <input v-model="form.company" type="text" class="input" placeholder="Mustermann GmbH" />
          </div>
          <div>
            <label class="label">Passwort</label>
            <input v-model="form.password" type="password" class="input" placeholder="Mindestens 8 Zeichen" required minlength="8" />
          </div>
        ...
        <p class="mt-4 text-center text-sm text-gray-500">
          Bereits ein Konto?
          <RouterLink to="/login" class="text-brand-700 font-medium hover:underline">Anmelden</RouterLink>
        </p>
      </div>
    </div>
  </div>
...
const form = ref({ full_name: '', email: '', company: '', password: '' })
...
async function handleRegister() {
  ...
  try {
    ...
    error.value = e.response?.data?.detail || 'Registrierung fehlgeschlagen'
  } finally {
    loading.value = false
  }
}
